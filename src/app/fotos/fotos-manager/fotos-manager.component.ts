...
@Component({
  ...
  imports: [IonContent, IonButtons, IonItem, IonList, IonLabel, IonButton, IonThumbnail, CommonModule]
})
  ...
  async tomarFoto() {
    try {
      ...
      if (webPath) {
        ...
        await this.storageService.uploadImage(webPath.toString(), this.user?.email || '');
